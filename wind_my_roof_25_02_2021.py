...
if __name__ == '__main__':
    ...
    C_e_command = 15
    V = 10
    om_t = om_theo(_C_r, _R, _k, V, C_e_command, _lamb)

    # ============| Affichage des données |=================
    _N_samples = 1000
    _V_array = np.array([V]*_N_samples)

    t, v, o, c, p = euler(f, 0, 20, _N_samples, 0, _V_array, C_e=C_e_command)
    print(f"La valeur finale expérimentale de omega est : {o[-1]:.3f}\nCelle de P est : {p[-1]:.3f}")
    print(f"La valeur finale théorique de omega est : {om_t:.3f}")
    print(f"Pourcentage d'erreur entre valeur théorique et valeur exp : {abs(o[-1]- om_t)*100:.2f}")

    print(f"Puissance moyenne : {np.average(p):.2f}")
    # display(t, v, o, c, p)

    # ============| Affichage des erreurs |=================
    # liste_Ce = np.arange(start=0, stop=81, step=1)
    # errors = []
    # print("Ce = ", end=" ")
    # for Ce in liste_Ce:
    #     print(Ce, end=" ")
    #     om_t_Ce = om_theo(_C_r, _R, _k, _V, Ce, _lamb)
    #     t, v, o, c, p = euler(f, 0, 20, _N_samples, 0, _V_array, C_e=Ce)
    #     errors.append(int(abs(o[-1] - om_t_Ce)*10000)/100)
    #
    # print("")
    # plt.plot(liste_Ce[10:], errors[10:])
    # plt.show()
